      ...
      <div className="mx-auto max-w-7xl px-6 py-24 md:px-12 md:py-32 lg:px-20">
        ...
        <div className="mt-10 grid gap-14 lg:grid-cols-[1.3fr_1fr] lg:gap-20">
          <div data-reveal-item>
            ...
            <p className="mt-4 max-w-xl text-sm leading-relaxed text-muted-foreground md:text-base">
              ...
              Agnirath Aerospace — on the MAGANAL rover mission.
            </p>
                ...
                {dataSheet.map((row) => {
                  ...
                  return (
                    <div
                      ...
                      onClick={
                        copyable ? () => copyRow(row.value, row.label.toUpperCase()) : undefined
                      }
                      onKeyDown={
                        copyable
                          ? (e) => {
                    ...
                    >
                      <dt className="mono-label !text-[9px] text-muted-foreground">{row.label}</dt>
            ...
            <div className="mt-5 max-w-xl rounded-xl border border-white/10 bg-white/[0.02] p-5">
              <p className="mono-label !text-[9px] text-white/40">SYSTEM LOG — LAST 3 EVENTS</p>
              <ul className="mt-4 space-y-2.5">
                {systemLog.map((entry) => (
                  ...
                  >
                    <span className="mono-label !text-[9px] text-mint">{entry.when}</span>
                    <span className="text-right text-[13px] text-white/85">{entry.what}</span>
                  </li>
                ))}
              </ul>
            </div>
          ...
          <div className="grid grid-cols-2 gap-4 self-start">
            {stats.map((stat) => (
              <div key={stat.label} data-reveal-item className="card-spotlight eclipse-card p-5">
                <p className="font-display text-3xl font-semibold text-white md:text-4xl">
                  {"text" in stat ? stat.text : <CountUp to={stat.to} decimals={stat.decimals} />}
                </p>
                <p className="mono-label mt-3 text-muted-foreground">{stat.label}</p>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="mt-10 grid gap-5 md:grid-cols-2">
          ...
          >
            ...
            <p className="mt-2 mono-label !text-[9px] text-muted-foreground">{maganal.period}</p>
            <p className="mt-4 text-sm leading-relaxed text-white/90">
              Team member on <span className="text-mint">{maganal.name}</span> — {maganal.full}. The
              project runs on {maganal.areas.slice(0, -1).join(", ")} and{" "}
          ...
          >
            ...
            <p className="mt-4 text-sm leading-relaxed text-white/90">
              <span className="text-violet">AnatomiaX</span> — a 3D AI-powered anatomy learning
              platform, on the build bench.
            </p>
            <p className="mt-4 text-sm leading-relaxed text-muted-foreground">
              Alongside: deepening Three.js, systems programming and mission-grade full-stack
              engineering.
            </p>
          </div>
        </div>
        ...
        <div className="mt-10 grid gap-3 sm:grid-cols-4">
          {[
            ...
            <div key={step.word} data-reveal-item className="card-spotlight eclipse-card p-4">
              <p className="font-display text-sm font-semibold text-mint">{step.word}</p>
              <p className="mt-2 text-[11px] leading-relaxed text-muted-foreground">{step.note}</p>
            </div>
          ))}
        </div>
        ...
        <div data-reveal-item className="mt-10">
          ...
          <div className="mt-4 flex flex-wrap gap-x-8 gap-y-2">
            <p className="mono-label !text-[9px] text-muted-foreground">
              MISSION LOG — <span className="text-mint">{maganal.name}</span> · {maganal.org}
            </p>
            <p className="mono-label !text-[9px] text-muted-foreground">
